...
function ReportSite(Site, Remove) {  
  ...
  if (/^\*/.test(Site)) {                        //Is it a *common site?
    ...
    if (/^[A-Za-z1-9\-]{2,63}\.(org\.|co\.|au\.)?[A-Za-z1-9\-]{2,63}$/.test(Site)) {                      
      ...
      Domain = Site.match(/[A-Za-z1-9\-]{2,63}\.(org\.|co\.|au\.)?[A-Za-z1-9\-]{2,63}$/)[0];     //Extract domain with optional double-barrelled tld
